...
import 'react-toastify/dist/ReactToastify.css';
  ...
  const columns = [
      ...
      {
        name: 'Preview',
        selector: row=> <img className='preview_img' src={'/public/uploads/'+row.slug} />
      },
      {
        name: 'Action',
        selector: row=> {
          ...
          return(
            <div>
              ...
              <button onClick={() => {if(window.confirm('Are you sure to delete this record?')){  Inertia.delete(route('file.destroy',row.id))  };}}  className="btn btn-datatable btn-icon btn-transparent-dark deleteBtn"><fIcon.Trash2 /></button>
            </div>
          )
...
useEffect(()=>{
 ...
 // console.log(selectedRows)
	...
	const contextActions = React.useMemo(() => {
      ...
      if (window.confirm(`Are you sure you want to delete ${selectedRows.length} item?`)) {
       ...
       // console.log(selectedItemString);
        Inertia.delete(route('file.destroy',selectedItemString))
        setToggleCleared(!toggleCleared);
			}
    ...
    <DashboardLayout>
        ...
        <div className="container-xl px-4">
            <div className="card mb-4">
                <div className="card-header">Files Information <Link className='btn btn-primary' style={{'float':'right'}} href={route('file.create')}>Add file</Link></div>
