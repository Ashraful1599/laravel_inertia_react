...
export default function ProductEdit({categories, tags, terms, attributes, product,skus}) {
...
if(product.tag){
    ...
    selectedTagOptions.push({value:  item.id, label: item.tag_name})
        //  tagOptions.push({label: item.tag_name})
    })
}
 const {data, setData, post, processing, errors, reset, progress} = useForm({
    ...
    'promo1': product.promo1!=null?product.promo1: "",
    'promo2': product.promo2!=null?product.promo2: "",
...
let editAttribute = {}
  ...
  skus.map((item, index)=>{
    ...
    item.var_products.map((el, i)=>{

        editAttribute[el.variant_id] = [el.variant_option_id];
    });
  })
...
if(product.product_type == 'variable'){
    product.variant.map((item)=>{
        var attName = item.attribute.name.toLowerCase();
        var filterTerm = terms.filter((trm)=> item.attribute_id == trm.attribute_id );
          var termArray = [];
          filterTerm.forEach((item)=>{
              termArray.push(item.id);
          })
          return attributesValNew[attName] = termArray;
      })
    
      //console.log(attributesValNew);
    ...
    attrs.map((item,i)=>{
         if(skus[i]){var price = skus[i].price}else{var price = ""};
         if(skus[i]){var sku = skus[i].sku}else{var sku = ""};
  ...
  const [inarr, setInarr] = useState(product.variant);
...
//    setArr(attrs);
  ...
  //  console.log('unchecked');
    ...
    var fileredItem =  inarr.filter((item)=>{
        if(item.name){
           return item.name.toLowerCase() != e.target.name
        }else{
          return  item.attribute.name.toLowerCase() != e.target.name
        }
                                ...
                                <div className="col-sm-8">
                                    {
                                        attributes.map((item, index)=>{
                                            if(product.variant[index]){
                                                if(product.variant[index].attribute.name.toLowerCase() == item.name.toLowerCase()){
                                                    return(
                                                        <div key={index} className="form-check form-check-inline">
                                                         <input defaultChecked onChange={variableCheck} name={item.name.toLowerCase()} className={'form-check-input variant_check variant_check'+item.name.toLowerCase()} type="checkbox" value={item.name.toLowerCase()} />
                                                ...
                                                return(
                                                    <div key={index} className="form-check form-check-inline">
                                                     <input onChange={variableCheck} name={item.name.toLowerCase()} className={'form-check-input variant_check variant_check'+item.name.toLowerCase()} type="checkbox" value={item.name.toLowerCase()} />
                                                     <label className="form-check-label" >{item.name}</label>
                                                    </div> 
                                                )
                                            }
                            ...
                            return (
                                <div key={i} className="row mt-3 variable_row">

                                     {
                                        ...
                                        inarr.map((item, index)=>{
                                            ...
                                            return(
                                                <>
                                                { item.name?
                                                       <div className={'col-sm attr_'+item.name.toLowerCase()}>
                                                      ...
                                                      item.attribute?
                                                        <div  className={'col-sm attr_'+item.attribute.name.toLowerCase()}>
                                                            <select required value={arritem[item.attribute.name.toLowerCase()]}  name={item.attribute.name.toLowerCase()} onChange={AddHandleChange} id={i} className={'form-select '+arritem[item.attribute.name.toLowerCase()]}>
                                                                <option className={"firstItem"+item.attribute.name.toLowerCase()} value={item.attribute.name.toLowerCase()}>Select {item.attribute.name}</option>
                                                                {
                                                                    item.variant_option.map((item, index)=><option key={index} value={item.term.id}>{item.term.name}</option>)
                                                                }
                                    ...
                                    <input
                                            ...
                                            name={'sku'}
                                        //    defaultValue={product.variant_value[i].sku}
                                            value={arritem.sku}
                                            id={i}
                                            type="text"
                                            size="40"
                ...
                <div className="card mb-4 nav-sticky">
                    ...
                    <div className="form-group">
                        <label className="col-sm-2 col-form-label">Category</label>
                            <select defaultValue={data.cat_id} className="form-control" onChange={inputHandle} name="cat_id" >
